...
Includes model training and evaluation loops.
"""
import os
import random
import numpy as np
...
def train_model(model, train_loader, val_loader, test_loader, args, device='cuda'):
    """Training loop (enhanced version)."""
    ...
    for epoch in range(args.epochs):
        ...
        # Evaluation (validation only; keep test set for post-training evaluation)
        ...
        if val_results['rmse'] < best_val_rmse:
            ...
            # Save best metrics (validation only)
            best_results = {
                'val': val_results
            }
